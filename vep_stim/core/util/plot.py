...
from vep_stim.core.util.nifti import gen_volume_regions
from vep_stim.core.util.elecs import NamedPoints, Contacts
...
def plot_t1_elec(t1_img_file, elec_img_file, elec_pos_file, contact_name, filename):
    sub_id = [_ for _ in t1_img_file.split('/') if _.startswith('sub-')][0]
    ...
    ax.text(0.1, 0.9, f"{contact_name} ({sub_id})", size=16, ha="left")
...
def plot_x_in_y(A_fname, B_fname, png_fname):
    A = nib.load(A_fname).get_data().astype('f')
    B = nib.load(B_fname).get_data().astype('f')
    if A.ndim == 4:
        A = A[..., 0]
    A = A / A.max()
    if B.ndim == 4:
    ...
    for i, n in enumerate(A.shape):
        r = np.r_[1, 0, 0].reshape((3, 1, 1, 1))
        b = np.r_[0, 0, 1].reshape((3, 1, 1, 1))
        im = (r * A + b * B).transpose((1, 2, 3, 0))
        im = im.take(n//2, axis=i)
        pl.subplot(2, 2, i + 1)
        pl.imshow(im)
        pl.xticks([])
        pl.yticks([])
    pl.suptitle(f'{A_fname} in {B_fname}')
    pl.savefig(png_fname)
    pl.close()


def plot_t1_in_b0(proc_dir):
    plot_x_in_y(
        f'{proc_dir}/dwi/bzero.nii.gz',
        f'{proc_dir}/dwi/T1_in_bzero.nii.gz',
        f'{proc_dir}/dwi/T1_in_bzero.png')


def plot_elec_in_t1(proc_dir):
    plot_x_in_y(
        f'{proc_dir}/elec/elec-CT_in_T1.nii.gz',
        f'{proc_dir}/mri/T1.RAS.RO.nii.gz',
        f'{proc_dir}/elec/elec_in_T1.png')
